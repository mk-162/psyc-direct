...
function CtaInlineCard({ data }: { data: CtaInlineData }) {
  return (
    <Card className="p-6 sm:p-8 border-none bg-[var(--brand-bg-tint)] h-full">
...
export const CtaInline = ({ data, compact }: { data: CtaInlineData; compact?: boolean }) => {
  if (!data.heading) return null;

  if (compact) {
    return <CtaInlineCard data={data} />;
  }

  return (
    <section className="py-12 lg:py-16 px-4 sm:px-6 lg:px-8 bg-background">
      <div className="max-w-7xl mx-auto">
        <CtaInlineCard data={data} />
      </div>
    </section>
  );
};
